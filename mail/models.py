...
class MailMessageBase(models.Model):
    ...
    categories = models.CharField(
        max_length=50, blank=True, help_text="A comma separated list of categories. i.e GameOn|Reminder|Results + Week#"
    )
    reminder = models.BooleanField(
        default=False, help_text="This is a game reminder messsage.  Only players opting for reminders will get it"
    )
    ...
    top_components = SortedManyToManyField(
        Component, blank=True, help_text=f"These appear just below the header image, above the main message"
    )
    bottom_components = SortedManyToManyField(
        Component, blank=True, help_text=f"These appear below the the main message"
    )
    created = models.DateTimeField(default=timezone.now)
    scheduled = models.DateTimeField(
        null=True, blank=True, help_text="If set then mail will go out at the scheduled time."
    )
...
class MailMessage(MailMessageBase):
    enable_blast = models.BooleanField(default=False, help_text="Must be check to send blast.")
    top_components = SortedManyToManyField(
        Component,
        blank=True,
    ...
    bottom_components = SortedManyToManyField(
        Component, blank=True, related_name="messages_bottom", help_text=f"These appear below the the main message"
    )
...
def add_mail_log(mail_message, batch_id, canceled=None):
    kwargs = model_to_dict(
        mail_message, exclude=["id", "series", "top_components", "bottom_components", "enable_blast", "sent", "tested"]
    )
