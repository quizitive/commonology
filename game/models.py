...
class Game(models.Model):
    ...
    google_form_url = models.CharField(max_length=255, blank=True, help_text="Enter the form url")
    ...
    def players_dict(self):
        ...
        return (
            self.game_questions.first()
            .raw_answers.filter(removed=False)
            .values("player", "player__display_name")
            .annotate(
                is_host=models.Case(
    ...
    def teams(self):
        return (
            self.questions.exclude(type=Question.op)
            .first()
            .raw_answers.values(team_id=models.F("player__teams"), team_name=models.F("player__teams__name"))
    ...
    def valid_raw_string_counts(self):
        return (
            Answer.objects.values("question_id", "raw_string")
            .filter(
                removed=False,
                question__game=self,
            )
            .annotate(count=models.Count("raw_string"))
            .order_by()
        )
    ...
    def coded_player_answers(self):
        answer_code_subquery = AnswerCode.objects.filter(
            raw_string=models.OuterRef("raw_string"), question__game=self, question=models.OuterRef("question")
        ).values("question", "coded_answer")
        return (
            Answer.objects.filter(question__game=self, removed=False)
            .exclude(question__type__in=(Question.op, Question.ov))
            .values_list("player", "player__display_name", "question__text")
            .annotate(
    ...
    @property
    def max_date(self):
        return self.questions.aggregate(models.Max("raw_answers__timestamp"))["raw_answers__timestamp__max"]

    @property
    def min_date(self):
        return self.questions.aggregate(models.Min("raw_answers__timestamp"))["raw_answers__timestamp__min"]
...
class Question(models.Model):
    game = models.ForeignKey(Game, null=True, on_delete=models.SET_NULL, related_name="questions", db_index=True)
    number = models.PositiveIntegerField(null=True)
    text = models.CharField(max_length=10000)
    ga = "GA"
    op = "OP"
    ov = "OV"
    QUESTION_TYPES = [(ga, "Game"), (op, "Optional"), (ov, "Optional (visible)")]
...
class Answer(models.Model):
    ...
    question = models.ForeignKey(Question, on_delete=models.CASCADE, related_name="raw_answers", db_index=True)
    raw_string = models.CharField(max_length=1000)
    removed = models.BooleanField(default=False, help_text="This answer should be removed from the thread")
...
class AnswerCode(models.Model):
    ...
    question = models.ForeignKey(Question, on_delete=models.CASCADE, related_name="coded_answers", db_index=True)
    raw_string = models.CharField(max_length=1000)
