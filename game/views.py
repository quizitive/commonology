...
class GameFormView(FormMixin, PSIDMixin, BaseGameView):
    ...
    def render_game(self, request, game, player, editable=True):
        ...
        self.game = game
        return render(
            request, "game/game_form.html", self.get_context(game, player.id, psid, dn_form, editable=editable)
        )
    ...
    def get(self, request, *args, **kwargs):
        ...
        form_data = {
            str(qid): answer
            for qid, answer in Answer.objects.filter(question__game=game, player=player).values_list(
                "question", "raw_string"
            )
        }
    ...
    def _build_game_forms_post(self, player):
        ...
        form_data = {
            qid: answer
            for qid, answer in zip(self.request.POST.getlist("question"), self.request.POST.getlist("raw_string"))
        }
    ...
    def render_answers_submitted_card(self, request, msg, player, game):
        msgs = {
            ...
            "duplicate": {
                ...
                "custom_message": mark_safe(
                    f"You have already submitted answers for this game. "
    ...
    def email_player_success(self, request, game, player):
        ...
        email_context = {"game_name": game.name, "url": f"https://{domain}{answers_url}"}
    ...
    def get_context(
        self, game, player_id=None, psid=None, dn_form=None, forms=None, editable=True, replay=False, **kwargs
    ):
    ...
    def _build_game_forms(self, game, form_data=(), player=None, editable=True):
        ...
        if form_data:
            forms = {
                q.id: QuestionAnswerForm(
                    question=q,
                    editable=editable,
                    auto_id=f"%s_{q.id}",
                    initial={"raw_string": form_data.get(str(q.id))},
                    data={"question": q, "raw_string": form_data.get(str(q.id)), "player": player},
                )
                for q in self.questions
            }
        else:
            forms = {q.id: QuestionAnswerForm(question=q, auto_id=f"%s_{q.id}") for q in self.questions}
        return forms

    def questions_with_forms(self, game, forms=None):
        forms = forms or self._build_game_forms(game)
        questions_with_forms = [(q, forms[q.id]) for q in self.questions.order_by("number")]
        return questions_with_forms

    def display_name_form(self, display_name, editable=True):
        return GameDisplayNameForm(editable=editable, initial={"display_name": display_name})
...
class GameEntryView(PSIDMixin, CardFormView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if g is None:
            msg = self.icon_message(
                icon="fa-solid fa-circle-questions", msg="Cannot find game. Perhaps you have a bad link."
            )
...
class GameEntryValidationView(PSIDMixin, CardFormView):
    ...
    def message(self, request, msg):
        ...
        return self.render(request, form=None, button_label="Home", form_method="get", form_action="/")
...
class QuestionSuggestionView(LoginRequiredMixin, CardFormView):
    ...
    custom_message = f"Suggest a question for a future game! Please supply " f"some example answers."
    button_label = "Submit"
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            p = request.user
            msg = (
                f"A new question has been suggested on the site:\n\n"
            ...
            messages.info(
                request,
                message=f"Thank you, your question has successfully been submitted. " f"Feel free to suggest another.",
            )
